...
- **Code** (`n8n-nodes-base.code`, v2) — used ten times: input validation, reserve-result classification, two controlled-response builders (rejected input, and the shared duplicate/finalized responder), three finalize-input builders, calendar-request building, calendar-result classification, and the final finalize-response builder.
...
- **Postgres** (`n8n-nodes-base.postgres`, v2.6) — used **two** times (`Reserve And Apply`, `Finalize Calendar Result`), `Execute Query` operation, every query fully parameterized (`$1, $2, ...` placeholders with a separate values array — never string-built SQL). See [Atomic idempotency/state-machine design](#atomic-idempotencystate-machine-design).
...
reserve AS (
  ...
  VALUES ($1, $2, $3, $4::timestamptz,
    CASE
      ...
      ELSE 'conflict'
    END)
  ON CONFLICT (reply_event_id) DO UPDATE SET reply_event_id = EXCLUDED.reply_event_id
  RETURNING reply_event_id, result_status, (xmax = 0) AS is_new
)
SELECT
  (SELECT count(*) FILTER (WHERE is_new) FROM reserve)::int AS reserved_count,
  (SELECT result_status FROM reserve WHERE is_new) AS new_result_status,
  (SELECT result_status FROM reserve WHERE NOT is_new) AS existing_result_status,
...
**Duplicate handling now returns the real, existing outcome — never a generic label.** When `reserved_count = 0` (the conflict branch fired instead of a fresh insert), the query still reports the *pre-existing* `result_status` for that `replyEventId` — `calendar_sync_pending`, `calendar_sync_failed`, `confirmed`, `conflict`, whatever it actually is. A pending event, a failed event, and a genuinely completed event are all distinguishable to the caller, instead of everything collapsing into one `"duplicate"` string.

**Why `ON CONFLICT ... DO UPDATE` (a deliberate no-op) instead of `DO NOTHING`:** an earlier version of this statement used `ON CONFLICT (reply_event_id) DO NOTHING`, then a separate plain `SELECT result_status FROM reply_events WHERE reply_event_id = $1` to recover the pre-existing row for the duplicate response. Under a genuine concurrent race — two requests carrying the *identical* `replyEventId` executing this statement at the same moment — that plain `SELECT` reads from the snapshot taken at the start of the *losing* transaction's statement, which was captured **before** the winning transaction committed. Reproduced directly against local Postgres 16.15 by holding one transaction open (`BEGIN; <statement>; pg_sleep(...)` before `COMMIT`) while a second, identical statement executed concurrently: the loser correctly got `reserved_count = 0`, but `existing_result_status` came back **`NULL`** instead of the winner's real, just-committed status — because `DO NOTHING` skips the row without forcing a fresh, MVCC-correct read of it. Switching to `DO UPDATE SET reply_event_id = EXCLUDED.reply_event_id` (a no-op write on the conflicting row) forces Postgres to actually touch and re-read that row via the same mechanism used for concurrent `UPDATE`s (`EvalPlanQual`), so `RETURNING` always reflects the true, current, committed row — whether this statement just inserted it or lost the race to another one. `(xmax = 0)` distinguishes the two cases in the same `RETURNING` clause: `xmax = 0` means this statement performed the actual insert; a non-zero `xmax` means it hit the conflict branch and is reading a row committed by someone else. Re-run against the same concurrent-race setup after the fix: the loser correctly returns the winner's real `result_status`, never `NULL`.
...
The single Postgres write-node downstream of the initial atomic write (`Finalize Calendar Result`, shared across the missing-calendar-event, Calendar-success, and Calendar-failure paths alike) applies its write with a **version guard**:
...
| 13 | Concurrent **identical** `replyEventId` (a true duplicate delivery racing itself) | The losing transaction's `existing_result_status` correctly returns the winner's real, committed status (e.g. `calendar_sync_pending`) — never `NULL` | direct SQL inspection against a local, isolated PostgreSQL 16.15 instance: one session held open (`BEGIN` ... before `COMMIT`) while an identical second session executed the same statement concurrently, confirming the fixed `ON CONFLICT ... DO UPDATE` + `xmax = 0` pattern; the prior `ON CONFLICT ... DO NOTHING` + separate `SELECT` design was confirmed, with the same method, to return `NULL` under this exact race |
| 14 | A second action while Calendar sync is pending | `calendar_busy`, zero additional Calendar calls | mock-bound copy |
| 15 | `confirmed → cancelled` Calendar transition | Both transitions succeed with the correct status | mock-bound copy |
| 16 | `cancelled → confirmed` restores `status: confirmed` | Confirmed via the mock server's own recorded event state: genuinely restored, not merely re-flagged | mock-bound copy |
| 17 | `actionTimestamp` stored and normalized correctly | A `Z` timestamp and an equivalent explicit-offset timestamp both normalize to the identical stored UTC instant in Postgres; an impossible timestamp is still rejected | direct SQL inspection + real committed file |
| 18 | Non-calendar actions (`reschedule_requested`/`manual_review`) remain atomic | Finalized in the same atomic write as the appointment mutation, zero Calendar calls | real committed file |
| 19 | Previous rejection/concurrency/status coverage (missing appointment, stale version, oversized/injection-shaped identifiers returning `null`, full Calendar HTTP-status matrix 400/401/404/429/500/timeout) | All pass, identical to prior verification | real committed file (rejections) + mock-bound copy (Calendar statuses) |
| 20 | Official CLI export/import | Exported via `n8n export:workflow`, imported into a clean instance, exported again — `settings`, `nodes`, `connections` byte-for-byte identical | real committed file |
| 21 | Complete rerun on a second clean n8n/Postgres instance | A genuinely separate, freshly-initialized n8n instance plus a freshly-created Postgres database reproduced the calendar-redirect-prevention, missing-calendar-event, reschedule/manual-review, duplicate, stale-version, missing-appointment, oversized-identifier, Calendar-400, concurrency, and full confirm→cancel→confirm-restore results identically | real committed file (rejections) + a fresh mock-bound copy built on that same clean instance (Calendar-requiring cases) |
| 22 | Execution-data persistence settings verified | Canary `replyEventId` immediately unreachable via the executions API for this workflow's own execution; confirmed physically present but soft-deleted in SQLite immediately after; confirmed physically purged following a process restart under accelerated pruning | real committed file |
| 23 | Controlled output contains no sensitive fields | Confirmed programmatically: no response contains message text, phone numbers, credentials, SQL, calendar payloads, or raw provider errors | real committed file + mock-bound copy |
