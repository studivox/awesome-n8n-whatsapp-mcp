...
6. **Do not point Meta's WhatsApp Cloud API webhook configuration directly at this URL.** This workflow only implements the POST message-handling side. Direct Meta integration additionally requires: (a) a separate endpoint handling Meta's `GET` verification handshake (`hub.mode`/`hub.verify_token`/`hub.challenge`), and (b) `X-Hub-Signature-256` signature validation on every inbound POST, before the URL is ever exposed publicly. Neither is included here — build and test those separately first.
