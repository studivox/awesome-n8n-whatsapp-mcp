...
- **Execution access through the n8n API is disabled and the execution is soft-deleted immediately.** Physical removal is handled separately by n8n's pruning configuration and was not directly observed during this workflow's own test window — see [Test procedure](#test-procedure) for exactly what was and wasn't confirmed.
