...
> [!NOTE]
> **Current status:** the curated resource foundation is live, and eight sanitized, tested workflow packages have been published. See [Current status](#current-status) below for exactly what that does and doesn't mean.
...
| ✅ | Eight sanitized, tested workflow packages are published — see [Available and planned workflows](#available-and-planned-workflows). None are described as production-ready or production-tested; all are verified templates with documented limitations. |
