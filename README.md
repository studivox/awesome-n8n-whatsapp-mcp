...
> [!NOTE]
> **Current status:** the curated resource foundation is live, and nine sanitized, tested workflow packages have been published. See [Current status](#current-status) below for exactly what that does and doesn't mean.
...
| ✅ | Ten sanitized, tested workflow packages are published — see [Available and planned workflows](#available-and-planned-workflows). None are described as production-ready or production-tested; all are verified templates with documented limitations. |
...
| Google Calendar ↔ Postgres synchronization | [`.json`](workflows/google-calendar-postgres-sync.json) · [`.md`](workflows/google-calendar-postgres-sync.md) | Available — sanitized, 27-scenario direct-SQL adversarial suite plus real-n8n-execution wiring tests, re-import verified on a second clean instance. A **reusable sub-workflow** (Execute Workflow Trigger) keeping one calendar in sync with Postgres in both directions: pull-sync (paginated, incremental-token-based, safe full-resync on `410 Gone`) refreshes drift-detection metadata only and never overwrites an appointment; an outbox drain pushes pending Postgres → Calendar creates/updates/cancels, idempotency-checked before every create and conditional (`If-Match`) before every update. A row-based lease (not `pg_advisory_lock` — see the workflow's own [Feasibility investigation](workflows/google-calendar-postgres-sync.md#feasibility-investigation)) prevents concurrent runners on the same calendar and self-heals on crash. Never matches events by name/title/phone/email — only by a stable, client-generated appointment id marker; never accepts a caller-controlled Calendar id or event id. Conflicts (a Calendar-side edit, an unrecognized event, a vanished mapping) are recorded, never silently resolved. Execution-data persistence disabled by workflow settings. The real Google Calendar API was never contacted during testing — verified only through a temporary, uncommitted mock-bound copy, disclosed in the workflow's own documentation. **Not** described as production-ready; see the workflow's own [Known limitations](workflows/google-calendar-postgres-sync.md#known-limitations). |
| Unpaid invoice reminder | [`.json`](workflows/unpaid-invoice-reminder.json) · [`.md`](workflows/unpaid-invoice-reminder.md) | Available — sanitized, 21-scenario test suite (direct SQL, live n8n execution, and CLI export/import), re-import verified on a second clean instance. A **reusable sub-workflow** (Execute Workflow Trigger) that attempts a single reminder for one already-decided invoice/reminder-stage pair — it does not decide reminder cadence itself. Verifies eligibility (genuinely unpaid, overdue, not already reminded at this stage) and reserves exactly one attempt atomically in Postgres, via a PL/pgSQL function modeled on the confirmation/cancellation workflow's proven exception-safe ownership gate, before calling the existing "WhatsApp template message sender" by its committed workflow id. Recipient phone, amount, currency, and due date are read exclusively from Postgres — never caller input. Postgres and WhatsApp are not one atomic transaction; a send failure after a successful reservation is reported honestly as `reminder_failed`, and a Postgres failure after a successful send is left `reminder_pending`/`reconciliation_required`, never false success and never an automatic resend. Execution-data persistence disabled by workflow settings. The real Meta/WhatsApp API was never contacted during testing — verified only through a temporary, uncommitted mock-bound copy, disclosed in the workflow's own documentation. **Not** described as production-ready; see the workflow's own [Known limitations](workflows/unpaid-invoice-reminder.md#known-limitations). |
| Customer quotation delivery | [`.json`](workflows/customer-quotation-delivery.json) · [`.md`](workflows/customer-quotation-delivery.md) | Available — sanitized, 29-scenario adversarial test suite (direct SQL, live n8n execution, and CLI export/import), re-import verified on a second clean instance. A **reusable sub-workflow** (Execute Workflow Trigger) that delivers an already-approved quotation's summary and a validated link via WhatsApp — it does **not** generate a PDF or any rendered document; the quotation content lives at a link hosted entirely by the quotation-owning business's own application. The caller supplies only `idempotencyKey`, `quotationId`, and `expectedVersion` — recipient phone, quotation number, amount, currency, valid-until date, and link token are read exclusively from Postgres, never caller input, and the link is built only from a fixed hardcoded HTTPS origin plus a strictly character-validated opaque token (never an arbitrary caller- or database-controlled URL). Money is represented as integer minor units, formatted with pure string arithmetic — never JS floating point. Reserves exactly one attempt atomically in Postgres via the same proven exception-safe ownership-gate pattern as the confirmation/cancellation and invoice-reminder workflows, before calling the existing "WhatsApp template message sender" by its committed workflow id. Postgres and WhatsApp are not one atomic transaction; a send failure after a successful reservation is reported honestly as `failed`, and a Postgres failure after a successful send is left `reconciliation_required`, never false success and never an automatic resend. Execution-data persistence disabled by workflow settings. The real Meta/WhatsApp API was never contacted during testing — verified only through a temporary, uncommitted mock-bound copy, disclosed in the workflow's own documentation. **Not** described as production-ready; see the workflow's own [Known limitations](workflows/customer-quotation-delivery.md#known-limitations). |
